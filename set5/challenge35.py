...
class EchoBot:
    ...
    def set_A(self, A):
        self.A = A
        b, B = get_dh_coefficient(self.p, self.g)
        s = modexp(self.A, b, self.p)
...
class EchoBotMITM:
    def __init__(self, echo_bot: EchoBot):
        ...
        self.key = get_key_from_s(1)

    def set_params(self, p, g):
        self.p = p
        self.g = 1
        self.echo_bot.set_params(self.p, self.g)
        return True

    def set_A(self, A):
        self.A = A
        B = self.echo_bot.set_A(A)
        return B
